			...
			tokenRsp.data.map { entry ->
				...
				UploadManager().put(
					...
					{ key, responseInfo, response ->
						...
						val s = "$key, $responseInfo, $response"
						//返回码处理
						dealQiNiuErrorCodeNew(responseInfo)
...
data class QiNiuUpLoadExceptionNew(val responseInfo: ResponseInfo) : Throwable()

fun dealQiNiuErrorCodeNew(responseInfo: ResponseInfo) {
	when {
		responseInfo.isOK -> {
			//上传成功
		}
